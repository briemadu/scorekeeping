...
A copy of optim.py with restricted values for the search (only those that
lead to best results on the previous search with fewer interations). Just
to make sure that these combinations are also considered.
"""
...
configs = {
    "algorithm": "bayes",
    "parameters": {
        "random_seed": {"type": "discrete", "values": [2204, 54321]},
        "sent_encoder": {"type": "categorical", "values": [
                            'paraphrase-mpnet-base-v2',
                            #'stsb-roberta-base-v2',
                        ]},
        "batch_size": {"type": "discrete", "values": [128, 512]},
        #"hidden_dim": {"type": "discrete", "values": [256]},
        "hidden_dim": {"type": "discrete", "values": [512, 1024]},
        "lr": {"type": "discrete", "values": [1e-3]},
        "dropout": {"type": "discrete", "values": [0.1]},
        "clip": {"type": "discrete", "values": [0.5, 1]},
    },
    "spec": {
        "metric": "best_val_acc",
        "objective": "maximize",
...
for experiment in opt.get_experiments(
    ...
    #params = config.args()
    name = make_logging_dir(params, optim='extra2'+str(n_optim))
    n_optim += 1
